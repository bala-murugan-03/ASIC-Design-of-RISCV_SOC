...
*/
  ...
  assign t0 = ((!writeback_exec_squash_i) ? writeback_exec_value_i : 32'bz);
  assign t1 = ((!writeback_mem_squash_i) ? writeback_mem_value_i : 32'bz);
  assign t2 = ((!writeback_csr_squash_i) ? writeback_csr_value_i : 32'bz);
  assign t3 = ((!writeback_muldiv_squash_i) ? writeback_muldiv_value_i : 32'bz);
  assign t4_ra = fetch_instr_i[19:15];
  assign t5_rb = fetch_instr_i[24:20];
  
  riscv_regfile reg_dut(
                            .clk_i(clk_i),                      
                            .rst_i(rst_i),
 ...
 always @(posedge clk_i) begin
    opcode_rd_idx_o <= fetch_instr_i[11:7];
    opcode_ra_operand_o <= m2;
    opcode_rb_operand_o <= m3;
    opcode_ra_idx_o <= t4_ra;
    opcode_rb_idx_o <= t5_rb;
end
...
always @(posedge clk_i ) begin
      ...
      if(fetch_valid_i) begin
          if ((op_code == 7'b1100011) || (op_code == 7'b0010011) || (op_code == 7'b0110011) || (op_code == 7'b1101111) || (op_code == 7'b1100111)|| (op_code == 7'b0110111)|| (op_code ==  7'b0010111) )
